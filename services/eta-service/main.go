...
import (
	...
	"github.com/faisalaffan/faisalaffan-design-system/services/eta-service/handler"
...
func main() {
	...
	etaCfg := model.DefaultConfig()
	svc := service.NewETAService(etaCfg, rdb)
	h := handler.NewETAHandler(svc)
	...
	go func() {
		log.Printf("eta-service listening on :%s", cfg.Port)
